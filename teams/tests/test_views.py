...
class TeamViewTest(TestCase):
    ...
    def test_can_create_tournament_with_team_names(self):
        ...
        self.assertEqual(self.response.data['phase'], self.created_tournament.phase)

    def test_can_create_tournament_with_correct_names(self):
        expected_amount_teams = 12
        created_teams = Team.objects.filter(group__tournament__pk=self.created_tournament.pk)
    ...
    def test_can_create_tournament_with_groups(self):
        ...
        created_groups = Group.objects.filter(tournament__pk=self.created_tournament.pk)
        for group in created_groups:
            self.assertIn(group.letter, expected_group_names)
            self.assertEquals(len(group.teams.all()), expected_amount_teams_per_group)
    ...
    def test_can_create_tournament_with_matches(self):
        ...
        created_matches = Match.objects.filter(tournament__pk=self.created_tournament.pk)
